...
%                                                                   %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [i,Bm,CM,ACC,Y,Gbest]=PSODecomposition(MaskDim1,PopSize,itr,Bm,Tr,Ts,Val,Options,TypeCL,Settings)
    Eva=[];Premature_Count=[];
    ...
    for i=1:length(Premature_Count)
        if (Eva_Last.bm(i)>=Eva.bm(i))
            ...
            if (Premature_Count(i)>5)
                 ...
                 LV=Pop(i).V;
                 x=W*LV(:);
                 x2=C1*r1*(Pop(i).Mask(:,1)-Pop(i).Pbest.Mask(:,1));
                 x3=C2*r2*(Pop(i).Mask(:,1)-Gbest.Mask(:,1));
                 NV=x+x2+x3;
                 xx=Pop(i).Mask(:,1)+NV;       
                 for k=1:size(xx,1)
                    if(xx(k,1)<1||xx(k,1)>size(Tr.P,1))
                        xx(k,1)=randi(size(Tr.P,1),1,1);
                    end
                 end
                 Pop(i).Mask(:,1)=xx;
                 Pop(i).V=NV;
